...
const Card = ({ home }) => {
    ...
    return (
        <div className='grid grid-cols-3 gap-4 p-4'>
            {data.map((item, index) => (
                <div className='bg-gray-800 rounded-xl p-4 justify-between flex flex-col  ' key={index}>
                    ...
                    <div className='mt-4 w-full flex items-center'>
                        <button className={`${item.status === "In Complete" ? "bg-red-400" : "bg-green-700"} bg-red-400 p-2 rounded w-3/6`}>{item.status} </button>
            ...
            {home === "true" && <div className='flex flex-col justify-center items-center rounded-sm p-4 text-gray-300 bg-gray-800 hover:scale-105 hover:cursor-pointer transition-all duration-300 '>
